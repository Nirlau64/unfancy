...
function renderError(container, message, retryFn) {
    ...
    container.innerHTML = `
        <div class="error-msg">
            ...
            ${retryFn ? `<button class="btn-retry">Erneut versuchen</button>` : ''}
        </div>`;
...
async function updateHomeStatus() {
    ...
    try {
        ...
        if (spData?.is_playing) {
            ...
            spEl.innerHTML = `<div class="live-status-item"><p>🎧 Laurin hört gerade:</p><div class="track-box"><a href="${escapeHTML(track.external_urls?.spotify || '#')}" target="_blank" style="display:flex;"><img src="${escapeHTML(track.album.images[0].url)}" alt="Cover"></a><div class="track-info"><strong>${escapeHTML(track.name)}</strong><span>${escapeHTML(track.artists.map(a=>a.name).join(', '))}</span></div></div></div>`;
            hasActivity = true;
        }
    } catch(e) {}
    try {
        ...
        if (stData?.game) {
            stEl.innerHTML = `<div class="live-status-item"><p>🎮 Laurin spielt gerade:</p><a href="https://store.steampowered.com/app/${stData.appid || ''}" target="_blank" class="live-steam-link">${escapeHTML(stData.game)}</a></div>`;
            hasActivity = true;
        }
    } catch(e) {}
...
async function initSteam() {
    ...
    try {
        ...
        profileDiv.innerHTML = `<div class="profile-header"><img src="${escapeHTML(profile.avatar)}" alt="Avatar"><div><strong>${escapeHTML(profile.persona)}</strong><br><a href="${escapeHTML(profile.profileurl)}" target="_blank">Steam-Profil öffnen</a></div></div>`;
        ...
        const top16 = validGames.sort((a,b) => b.playtime_minutes - a.playtime_minutes).slice(0, 16);
        
        statsDiv.classList.add('section-relative');
        let html = `<div class="splash-bg"></div><div class="splash-content"><h2 class="text-center">Gesamtspielzeit: ${minutesToHours(totalMins)} Stunden</h2><p class="text-center text-muted" style="margin-top:-10px;">${unplayed} ungespielte Spiele im Backlog</p><h3 class="text-center">Meistgespielte Spiele</h3><div class="steam-grid">`;
        top16.forEach(g => {
            html += `<a href="https://store.steampowered.com/app/${g.appid}" target="_blank" class="steam-game-card" data-splash="https://shared.akamai.steamstatic.com/store_item_assets/steam/apps/${g.appid}/library_hero.jpg"><img src="https://cdn.cloudflare.steamstatic.com/steam/apps/${g.appid}/capsule_231x87.jpg" onerror="this.src='https://media.steampowered.com/steamcommunity/public/images/apps/${g.appid}/${g.img_logo_url}.jpg'" alt="${escapeHTML(g.name)}"><div class="steam-game-title">${escapeHTML(g.name)}</div><div class="text-muted">${minutesToHours(g.playtime_minutes)} Std.</div></a>`;
        });
        html += `</div>`;
        if (achievementsRes?.rarest?.length) {
            html += `<hr><h3 class="text-center">Seltenste Errungenschaften</h3><div class="achievement-grid">`;
            achievementsRes.rarest.forEach(a => {
                html += `<div class="achievement-card"><img src="${escapeHTML(a.icon)}" alt="${escapeHTML(a.name)}"><div class="achievement-info"><span class="achievement-name">${escapeHTML(a.name)}</span><span class="achievement-game">${escapeHTML(a.gameName)}</span></div><span class="achievement-rarity">${a.percent}%</span></div>`;
            });
            html += `</div>`;
        }
        html += `</div>`;
        ...
        const top10 = validGames.slice(0, 10);
        const top10Mins = top10.reduce((s, g) => s + g.playtime_minutes, 0);
        const ctx = document.getElementById('steam-chart').getContext('2d');
        steamChartInstance = new Chart(ctx, { type: 'pie', data: { labels: [...top10.map(g => g.name), 'Andere'], datasets: [{ data: [...top10.map(g => g.playtime_minutes), (totalMins - top10Mins)], backgroundColor: ['#7dd3fc','#3b82f6','#8b5cf6','#d946ef','#f43f5e','#f97316','#facc15','#4ade80','#2dd4bf','#a3e635','#6b7280'], borderColor: '#181a20', borderWidth: 2 }] }, options: { plugins: { legend: { position: 'bottom', labels: { color: 'white' } } } } });
    } catch (e) { renderError(profileDiv, "Steam-Daten konnten nicht geladen werden.", initSteam); }
}
...
async function loadSpotifyData(range) {
    ...
    try {
        ...
        (artists?.items || []).forEach(a => {
            aHtml += `<a href="${a.external_urls.spotify}" target="_blank" class="media-card artist" data-splash="${a.images[0]?.url || ''}"><img src="${escapeHTML(a.images[0]?.url || '')}" alt="${escapeHTML(a.name)}"><span>${escapeHTML(a.name)}</span></a>`;
        });
        ...
        (tracks?.items || []).forEach(t => {
            tHtml += `<a href="${t.external_urls.spotify}" target="_blank" class="media-card track" data-splash="${t.album.images[0]?.url || ''}"><img src="${escapeHTML(t.album.images[0]?.url || '')}" alt="${escapeHTML(t.name)}"><span>${escapeHTML(t.name)}</span><span class="artist-name">${escapeHTML(t.artists.map(ar => ar.name).join(', '))}</span></a>`;
        });
        tracksDiv.innerHTML = tHtml + `</div></div>`;
        setupSplashHover(tracksDiv, '.media-card', '.splash-bg');
    } catch(e) { renderError(artistsDiv, "Spotify-Daten konnten nicht geladen werden.", () => loadSpotifyData(range)); }
}
...
async function initLoL() {
    ...
    try {
        ...
        if (data.recentMatches?.length) {
            fullHTML += '<h3 class="text-center">Letzte 10 Spiele</h3><div class="lol-table-wrap"><table><thead><tr><th>Champion</th><th>K/D/A</th><th>CS</th><th>Dauer</th><th>Modus</th><th>Ergebnis</th></tr></thead><tbody>';
            data.recentMatches.slice(0, 10).forEach(m => {
                ...
                const splashUrl = cd ? `https://ddragon.leagueoflegends.com/cdn/img/champion/splash/${cd.img.split('/').pop().replace('.png', '')}_0.jpg` : '';
                fullHTML += `<tr class="lol-match-row" data-splash="${splashUrl}"><td>${cd ? `<img src="${cd.img}" alt="${cd.name}">` : ''} ${escapeHTML(cd ? cd.name : m.you.championId)}</td><td>${m.you.kills}/${m.you.deaths}/${m.you.assists}</td><td>${m.you.cs ?? '-'}</td><td>${m.gameDuration ? `${Math.floor(m.gameDuration/60)}:${('0'+(m.gameDuration%60)).slice(-2)}` : '-'}</td><td>${escapeHTML(queueMap[m.queueId] || m.gameModeReadable || m.gameMode || '-')}</td><td style="color:${res==='Sieg'?'#4ade80':(res==='Niederlage'?'#f43f5e':'inherit')}">${res}</td></tr>`;
            });
    ...
    } catch(e) { renderError(statsContainer, "LoL-Daten konnten nicht geladen werden.", initLoL); }
}
...
async function initSocials() {
    ...
    try {
        ...
        if (ytData.status === 'ok' && ytData.items.length > 0) {
            ...
            ytContainer.innerHTML = `<div class="yt-wrap"><iframe src="https://www.youtube-nocookie.com/embed/${escapeHTML(vidId)}?rel=0&modestbranding=1&origin=${encodeURIComponent(window.location.origin)}" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>`;
        }
    } catch(e) { renderError(ytContainer, "YouTube konnte nicht geladen werden.", initSocials); }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleSwipe() {
        ...
        let targetIndex = -1;
        if (touchStartX - touchEndX > swipeThreshold) targetIndex = (currentIndex + 1) % navLinks.length;
        else if (touchEndX - touchStartX > swipeThreshold) targetIndex = (currentIndex - 1 + navLinks.length) % navLinks.length;
        if (targetIndex !== -1) loadPage(navLinks[targetIndex].getAttribute('href'));
    }
    ...
    setInterval(() => { if (document.getElementById('home')) updateHomeStatus(); }, 30000);
});
